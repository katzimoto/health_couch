...
**Reads vs. Garmin sync are separated:** producing a plan/report never depends on a Garmin network sync. `Coach.get_health_context_for_report(mode="morning"|"evening")` always reads local Health Coach data (profile, analyzer summaries of cached Garmin metrics, today's nutrition/hydration/plans/health events, recent feedback) and both `morning_plan`/`evening_report` go through it — so throttled/failed/stale Garmin sync still yields a personalized report from cached data, never a generic fallback while any local data exists (`has_usable_data`). Only the Garmin *network* sync is rate-limited: `sync_policy.py` (pure, unit-tested) holds `evaluate_sync` (default 60-min interval, `force` lowers the floor to 10 min but never removes it; returns `{"synced": false, "reason": "throttled_recent_sync", …}` when skipped) and `classify_freshness` (none/fresh/cached/stale). `sync_garmin` (MCP) is the only throttled entry point; every local read tool is unthrottled. Intervals are configurable via `DEFAULT_MIN_SYNC_INTERVAL_MINUTES`/`FORCE_SYNC_MIN_INTERVAL_MINUTES`.
